...
# 🏛️ Mô Hình Biến Đổi dbt Medallion (`analytics_dbt/`)

Thư mục `analytics_dbt/` chứa mã nguồn dbt Core kết nối `dbt-duckdb` quản lý quá trình biến đổi dữ liệu chuẩn Medallion Architecture, kiểm thử chất lượng và tài liệu hóa schema (`docs.md`).
...
1. **Bronze Layer (`RAW`)**: Tầng dữ liệu thô nạp từ S3 CSVs.
