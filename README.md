...
# 🍕 AI & Data Engineering Intelligence Platform
...
[![Streamlit App](https://static.streamlit.io/badges/streamlit_badge_black_white.svg)](https://text-to-analytical-insights-by-phuccancode.streamlit.app/)
[![GitHub Actions CI](https://github.com/Phuccancode/text-to-analytical-insights/actions/workflows/ci_pipeline.yml/badge.svg)](https://github.com/Phuccancode/text-to-analytical-insights/actions)
[![Daily Incremental Ingestion](https://github.com/Phuccancode/text-to-analytical-insights/actions/workflows/daily_incremental_pipeline.yml/badge.svg)](https://github.com/Phuccancode/text-to-analytical-insights/actions)

> 🚀 **Live Production Web Application**: Experience the interactive platform live on Streamlit Cloud:
> **👉 [https://text-to-analytical-insights-by-phuccancode.streamlit.app/](https://text-to-analytical-insights-by-phuccancode.streamlit.app/)**
>
...
1. **Raw Data Generation & Daily Incremental Ingestion**: Synthetic relational food delivery datasets generated across 7 tables (`restaurants`, `users`, `food`, `menu`, `orders`, `order_items`, `reviews`) plus scheduled daily incremental random batch ingestion (`scripts/generate_daily_incremental_data.py`).
2. **Data Landing Lake (AWS LocalStack S3)**: Emulated AWS S3 storage (`s3://analytics-data-lake/raw/`) managed via Docker Compose.
3. **Analytical Data Warehouse (DuckDB)**: Embedded high-performance OLAP data warehouse (`data/warehouse/analytics_dw.duckdb`).
...
- **Automated Airflow DAGs**: Visual graph view for end-to-end batch ingestion (`end_to_end_batch`) and daily incremental batch refreshes (`daily_incremental_ingestion`).
...
```
data/warehouse/analytics_dw.duckdb
 ├── RAW (Bronze)       --> Direct raw table load from LocalStack S3 / CSVs
 ...
 └── AI (Enriched)      --> Gemini LLM review sentiment & aspect enrichment (REVIEW_ENRICHED)
```
