...
def enrich_reviews_pipeline():
    ...
    # Ensure schema AI exists
    con.execute("CREATE SCHEMA IF NOT EXISTS AI;")
    ...
    if reviews_df.empty:
        print("⚠️ No reviews found in STAGING.stg_reviews!")
        con.close()
        return
    ...
    # Register temp table and create AI.REVIEW_ENRICHED
    con.execute("DROP TABLE IF EXISTS AI.REVIEW_ENRICHED;")
    con.execute(
        """
        CREATE TABLE AI.REVIEW_ENRICHED (
            review_id INT,
            order_id INT,
            user_id INT,
            restaurant_id INT,
    ...
    count = con.execute("SELECT COUNT(*) FROM AI.REVIEW_ENRICHED;").fetchone()[0]
    con.close()
    print(f"✅ LLM Enrichment Complete! Created 'AI.REVIEW_ENRICHED' with {count} rows.")
