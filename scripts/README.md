...
generate_sample_data.py                  # Synthetic food delivery dataset generator (root)
```
...
1. **`generate_sample_data.py`**: Generates full synthetic food delivery dataset (7 CSV tables).
2. **`generate_daily_incremental_data.py`**: Appends random daily rows (users, orders, order items, reviews) to dataset.
3. **`init_localstack_s3.py`**: Uploads CSV datasets to AWS LocalStack S3 (`s3://analytics-data-lake/raw/`).
4. **`load_raw_duckdb.py`**: Populates `RAW` Bronze layer in DuckDB.
