...
DB_PATH = "data/warehouse/analytics_dw.duckdb"

st.title("🍕 AI & Data Engineering Intelligence Platform")
st.markdown(
...
try:
    ...
    total_reviews = con.execute("SELECT COUNT(*) FROM AI.REVIEW_ENRICHED;").fetchone()[0]
...
with col_a:
    ...
    1. **Data Ingestion (Local S3)**: Synthetic food delivery dataset generator uploaded to **AWS LocalStack S3** (`s3://analytics-data-lake/raw/`).
    2. **Medallion Data Warehouse**: **DuckDB** staging & gold schemas (`RAW` → `STAGING` → `MARTS`).
    3. **dbt Transformation**: Modular SQL models with 35+ data quality tests (`dbt build`).
    4. **AI Enrichment**: **LLM sentiment & topic extraction** into `AI.REVIEW_ENRICHED`.
...
with col_b:
    ...
    st.success("✅ AWS LocalStack S3 Bucket: `analytics-data-lake`")
    st.success("✅ DuckDB Database: `analytics_dw.duckdb`")
