...
1. **Khởi Tạo Dữ Liệu & Daily Incremental**: Tự động sinh dữ liệu thô qua 7 bảng (`restaurants`, `users`, `food`, `menu`, `orders`, `order_items`, `reviews`) và chạy job phát sinh dữ liệu ngẫu nhiên hàng ngày (`scripts/generate_daily_incremental_data.py`).
2. **Lưu Trữ Dữ Liệu Thô (AWS LocalStack S3)**: Giả lập AWS S3 (`s3://analytics-data-lake/raw/`) chạy qua Docker Compose.
3. **Kho Dữ Liệu Phân Tích (DuckDB)**: Embedded OLAP Data Warehouse tốc độ cao (`data/warehouse/analytics_dw.duckdb`).
...
- **Lập Lịch Tự Động Airflow**: Giao diện trực quan cho 2 DAGs (`end_to_end_batch` và `daily_incremental_ingestion`).
...
data/warehouse/analytics_dw.duckdb
 ├── RAW (Bronze)       --> Nạp trực tiếp từ LocalStack S3 / CSVs
 ...
 └── AI (Enriched)      --> Gemini LLM sentiment & aspect analysis (REVIEW_ENRICHED)
```
